...
namespace HTC.UnityPlugin.StereoRendering
{
    ...
    public class StereoRenderer : MonoBehaviour
    {
        ...
        private List<GameObject> recursiveStereoCameras = new List<GameObject>();
        ...
        private void CreateStereoCameraRig() {
            for (int i = 0; i < recursiveDepth; ++i) {
                GameObject cameraHead = new GameObject("Stereo Camera Head " + i.ToString());
                cameraHead.transform.parent = transform;

                GameObject stereoCameraEyeObj = new GameObject("Stereo Camera Eye " + i.ToString());
                stereoCameraEyeObj.transform.parent = cameraHead.transform;
                Camera cameraEye = stereoCameraEyeObj.AddComponent<Camera>();
                cameraEye.enabled = false;

                recursiveStereoCameras.Add(cameraHead);
            }

            stereoCameraHead = recursiveStereoCameras[0];
            stereoCameraEye = stereoCameraHead.GetComponentInChildren<Camera>();
        }
        ...
        private Vector4 GetObliqueNearClipPlane()
        {
            ...
            if (!isMirror)
            {
                ...
                clipPlaneCameraSpace = GetCameraSpacePlane(stereoCameraEye, anchorPos, anchorTransform.TransformDirection(Vector3.forward));
            }
            else
            {
                // get reflection plane -- assume +y as normal
